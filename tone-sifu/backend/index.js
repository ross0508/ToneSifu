...
app.get("/words/cmn/:id", async (req, res) => {
  ...
  const word = await pool.query(
    "SELECT * FROM mandarinword WHERE word_id = $1",
    [id]
  );
  res.json(word.rows);
});
...
app.get("/words/cmn/random/:n", async (req, res) => {
  ...
  const words = await pool.query(
    "SELECT * FROM mandarinword WHERE tone = ANY($1::int[]) ORDER BY RANDOM() LIMIT $2",
    [toneArray, n]
  );
  res.json(words.rows);
});
...
app.get("/words/yue/:id", async (req, res) => {
  const { id } = req.params;
  const word = await pool.query(
    "SELECT * FROM cantoneseword WHERE word_id = $1",
    [id]
  );
  res.json(word.rows);
});
...
app.get("/words/yue/random/:n", async (req, res) => {
  ...
  const toneArray = req.query.tones.map(Number); // Convert tones sent in params to array of numbers
  const words = await pool.query(
    "SELECT * FROM cantoneseword WHERE tone = ANY($1::int[]) ORDER BY RANDOM() LIMIT $2",
    [toneArray, n]
  );
  res.json(words.rows);
});
...
app.get("/users/:user_id", async (req, res) => {
  ...
  try {
    const userResult = await pool.query(
      "SELECT exp, exp_yue, exp_cmn FROM users WHERE user_id = $1",
      [user_id]
    );
